...
describe("relight Photoshop boundaries", () => {
  const fullSource: RelightSource = {
  ...
  it("captures the whole canvas and always disposes image data", async () => {
    ...
    await expect(captureRelightSource()).resolves.toEqual({
      ...fullSource,
  ...
  it("rejects a changed source document or canvas", async () => {
    ...
    await expect(validateRelightSource(fullSource)).rejects.toThrow("活动文档已切换");
    boundary.bridge.photoshop.app.activeDocument = { id: 7, width: 101, height: 80 };
    await expect(validateRelightSource(fullSource)).rejects.toThrow("画布尺寸已变化");
  });
