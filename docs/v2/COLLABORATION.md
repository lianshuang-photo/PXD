...
The foundation implementation PR targets `main` and includes the planning documents from draft PR #50. This lets the first reviewed change install its own CI and acceptance gate together with the application; a documentation-only bootstrap does not need a fabricated host check. PR #50 remains the earlier planning record until the integrated PR is reviewed. Neither PR is merged automatically.
