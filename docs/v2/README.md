...
更新：2026-09-20。状态：**首批模块已组装并拆为 PR #52–#58；原 #51 已被替代，真机验收仍不完整。** 见[模块 PR 索引](MODULAR-DELIVERY.md)、[实施状态](IMPLEMENTATION-STATUS.md)与[协作规则](COLLABORATION.md)。
...
- V2 0.1.6 的专业页和 Agent 已接到同一套草稿／任务服务，提供 17 个 Studio 工具和保留的 7 个 PS 工具。首批能力为图像编辑及已有层的名称／不透明度／可见性；真实供应商生成仍待验收。
- 121 份配方的内容、101 个数值参数及来源哈希已接入共享草稿。旧 PXD 其他玩法仍是复用候选，不能计为本轮交付。
- 实施已拆分为 [#52–#58 模块 PR](MODULAR-DELIVERY.md)，每份有明确基底、范围和测试。原 PR #51 保留历史集成记录，Alpha 固定在 `ls-studio-v2-alpha.0`。`V2` 是产品代际，未把包版本改成 2.0.0。

## 本轮交付与启动条件

已完成仓库评估、Alpha 保留、首批模块实现、独立交叉 review 和组装。本轮本机 193 项测试通过；真实 Photoshop 的抓图、羽化蒙版、组合属性修改、撤销及后续修改冲突已有[部分验收记录](evidence/2026-09-20-host-partial.md)。

G00–G07 代码已集成，G08 尚未通过；缺真实供应商、插件面板交互及完整样片验收。main 已要求独立 GitHub review、自动化和 computer-use 状态；CI 产物与受门禁约束的 draft release 流程已建立。后续仍按[实施计划](EXECUTION-PLAN.md)推进。
