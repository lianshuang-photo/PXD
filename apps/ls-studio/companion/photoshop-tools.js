...
const studioDefinitions = [
  ...
  ['studio_derive_draft', 'deriveDraft', 'Create a NEW shared image-edit draft from an immutable job snapshot. original preserves source, mask, references, settings, compiled prompt and recipe version. An omitted model is filled only from the recorded job provider model; conflicting explicit and recorded models fail. candidate-reference additionally appends this job result as a reference-role image; it never replaces the Photoshop capture. No generation or placement occurs. Read/edit the returned draft before explicitly running it; inherited autoApply still applies to that later run.', false, false],
