...
class PhotoshopBridge extends EventEmitter {
  ...
  request(name, args) {
    ...
    if (!this.status().connected) return Promise.reject(new Error("Photoshop 未连接。请打开 Photoshop 并加载 LS Studio 面板。"));
    if (this.jobs.size >= 8) return Promise.reject(new Error("Photoshop 正在处理其他请求，请稍后重试"));
    return new Promise((resolve, reject) => {
      const id = randomUUID();
      const timer = setTimeout(() => { this.jobs.delete(id); reject(new Error("Photoshop 工具响应超时，请确认没有模态对话框并重试")); }, this.timeoutMs);
      this.jobs.set(id, { id, tool: name, args, delivered: false, resolve, reject, timer, expiresAt: Date.now() + this.timeoutMs });
      this.emit("job");
    });
  }
  result(body, token) {
    ...
    if (result.ok) job.resolve(result); else job.reject(new Error(String(result.error || "Photoshop 执行失败").slice(0,600)));
    console.log(JSON.stringify({ time: new Date().toISOString(), event: "photoshop.tool", tool: job.tool, ok: result.ok }));
  }
  disconnect() {
    for (const job of this.jobs.values()) { clearTimeout(job.timer); job.reject(new Error("Photoshop 连接已中断，本次工具请求未完成")); }
    this.jobs.clear();
