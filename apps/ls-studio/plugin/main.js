...
(function () {
  ...
  function updatePrimaryLabel() {
    ...
    if (go) go.textContent = selScope() === "scope" ? "→" : "→";
  ...
  function paintRecordList(list) {
    ...
    for (let i = 0; i < taskRecords.length; i++) {
      ...
      btn.type = "button";
      btn.className = "record-rerun";
      btn.textContent = "重跑";
      btn.addEventListener("click", function () { rerunRecord(rec); });
  ...
  async function readDocumentState() {
    const a = app();
    const doc = a && a.activeDocument;
    const sel = await readSelectionBounds();
    if (!doc) {
      return { open: false, selection: sel, note: "no active document" };
    }
  ...
  function markWide() {
    const app = $("app");
    if (!app) return;
    const w = (app.getBoundingClientRect && app.getBoundingClientRect().width) || document.body.clientWidth || 360;
  ...
  document.querySelectorAll("#emptyChips .human-chip").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const sentence = btn.getAttribute("data-send") || btn.textContent.replace(/^\[|\]$/g, "");
      $("prompt").value = sentence;
      hideEmptyChips();
      try {
        await runPipeline(sentence);
      } catch (e) {
        log(String(e && e.message ? e.message : e), "err");
      }
    });
  ...
  document.querySelectorAll(".composer").forEach(function (el) { el.addEventListener("paste", onPasteImages); });
  ...
  document.querySelectorAll("input[name=defRet]").forEach(function (r) {
    r.addEventListener("change", function () {
      ...
      if (i !== undefined && rets[i]) rets[i].checked = true;
      r.parentElement && document.querySelectorAll("input[name=defRet]").forEach(function (x) {
        if (x.parentElement) x.parentElement.classList.toggle("on", x.checked);
      });
    });
  });
  ...
  if ($("toAgent")) {
    $("toAgent").addEventListener("click", function () {
      showTab("agent");
      const compile = $("compile");
      log("已从专业 IDE 带入当前编译", "ok");
      if (compile && compile.value) log(compile.value, "ok");
    });
  }
  ...
  if (typeof ResizeObserver !== "undefined") {
    try { new ResizeObserver(markWide).observe($("app") || document.body); } catch (_) {}
  }
